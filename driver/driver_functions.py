...
def run_hind_LV4(t1str,pkl_fnm):
    ...
    if not_testing == True: 
        ...
        if MI['lv4_use_restart']==0:
            ...
            print('we are now saving ATM LV4 to ' + fn_atm_out + ' ...')
            t01 = datetime.now()
            cmd_list = ['python','-W','ignore','atm_functions.py','atm_roms_dict_to_netcdf',str(level),pkl_fnm] # matt's new version adds: ,'hind' as last command in cmd_list
            os.chdir('../sdpm_py_util')
            ret5 = subprocess.run(cmd_list)   
            print('return code: ' + str(ret5.returncode) + ' (0=good)')  
            os.chdir('../sdpm_py_util')
            ...
            # changing for different river forcing: 
